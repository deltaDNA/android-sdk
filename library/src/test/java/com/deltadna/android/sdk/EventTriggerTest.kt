...
class EventTriggerTest {
    ...
    private lateinit var etcMetricStore: EventTriggeredCampaignMetricStore
    ...
    fun `missing attributes are replaced with sane defaults`() {
        with(EventTrigger(ddna, 0, jsonObject().convert(), etcMetricStore)) {
    ...
    fun `triggers are ordered based on priorities`() {
        val top = EventTrigger(ddna, 0, jsonObject("priority" to 2).convert(), etcMetricStore)
        val middle = EventTrigger(ddna, 0, jsonObject("priority" to 1).convert(), etcMetricStore)
        val bottom = EventTrigger(ddna, 0, jsonObject("priority" to 0).convert(), etcMetricStore)
    ...
    fun `triggers are ordered based on indices if priorities are same`() {
        val top = EventTrigger(ddna, 0, jsonObject("priority" to 0).convert(), etcMetricStore)
        val middle = EventTrigger(ddna, 1, jsonObject("priority" to 0).convert(), etcMetricStore)
        val bottom = EventTrigger(ddna, 2, jsonObject("priority" to 0).convert(), etcMetricStore)
        val expected = listOf(top, middle, bottom)
    ...
    fun `trigger is evaluated indefinitely by default`() {
        with(EventTrigger(ddna, 0, jsonObject("eventName" to "a").convert(), etcMetricStore)) {
            val event = KEvent("a")
    ...
    fun `evaluation fails against event with different name`() {
        assertThat(EventTrigger(ddna, 0, jsonObject("eventName" to "a").convert(), etcMetricStore)
    ...
    @Test
    fun `evaluation fails when enablers are present but none are satisfied`() {
        whenever(etcMetricStore.getETCExecutionCount(9989L)).thenReturn(10)
        with(EventTrigger(
                ddna,
                0,
                jsonObject(
                        ...
                        .convert(), etcMetricStore))
        {
            ...
            val event = KEvent("name")
            assertFalse { evaluate(event) }
    ...
    @Test
    fun `evaluation succeeds when enablers are present and at least one is satisfied`() {
        whenever(etcMetricStore.getETCExecutionCount(9989L)).thenReturn(10)
        with(EventTrigger(
                ddna,
                0,
