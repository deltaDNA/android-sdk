...
```
where `this` refers to the instance of the `Activity` from which you are
performing the request, and `MY_REQUEST_CODE` is a constant code which you
will need to define as it will be needed later on. Following this request
the SDK will automatically show the Image Message, without you having to
write extra code.
